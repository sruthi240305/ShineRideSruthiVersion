    ...
    const handleSubmit = (e) => {
        ...
        if (location.state?.vehicle?.id) {
            const updated = vehicles.map(v =>
                v.id === location.state.vehicle.id
                    ? { ...v, make: formData.make, model: formData.model, license: formData.licensePlate }
                    : v
            );
            localStorage.setItem('temp_vehicles', JSON.stringify(updated));
        }
